...
class Dobot_APP:
    ...
    def SnapshotAndConv(self, values):
        ...
        result, img = Snapshot(self.capture) # 静止画を撮影する。
        if result == 0:
            return 3, None

        sg.popup('スナップショットを撮影しました。', title='スナップショット')
        self.IMAGE_org = img.copy() # 撮影した画像を保存する
        ...
        if values['-GlobalThreshold-']:
            ...
            # ------------------ #
            if values['-OTSU-']:
                img = OtsuThreshold(img, values['-Gaussian-'])
            # 選択されていない　⇒　大局的閾値処理を行う場合
            else:
                threshold = float(values['-threshold-'])
                self.WebCam_err, img = GlobalThreshold(img, values['-Gaussian-'], threshold, Type=Type)
        ...
        # -------------------- #
            ...
            img = AdaptiveThreshold(img, values['-Gaussian-'], method, Type, block_size=block_size, C=const)
        ...
        # -------------------- #
            ...
            result, img = TwoThreshold(img, values['-Gaussian-'], LowerThresh, UpperThresh, color, Type)
            # エラー判定
            if result == 0:
                return 6, None
            elif result == 1:
    ...
    def Layout(self):
        ...
        Alignment = [
            ...
            [sg.Button('Set', size=(7, 1), key='-Set_x1-'),
             sg.Button('Set', size=(7, 1), key='-Set_x2-')],
        ...
        WebCamConnect = [
            ...
            [sg.InputCombo(('TOSHIBA_Web_Camera-HD',
             ...
             sg.InputCombo(('640x480',
                            ...
                            '160x120',
                            '1280x720',
                            '1280x800',), size=(11, 1), key='-WebCam_FrameSize-', readonly=True)],
    ...
    def Event(self, event, values):
        ...
        # Dobotの現在の姿勢を取得し表示する
            ...
            if self.connection == 0:
                ...
                self.DOBOT_err, pose = GetPose_click(self.api)
                if self.DOBOT_err == 1:
            ...
            if self.connection == 0: # Dobotが接続されていない時
                sg.popup('Dobotに接続していません。', title='Dobotの接続')
                return
            else:
                self.DOBOT_err, pose = GetPose_click(self.api)
            ...
            x = self.Alignment_1[0] + self.COG_Coordinate[1] * (self.Alignment_2[0] - self.Alignment_1[0]) / float(values['-IMAGE_height-'])
            print(x)
        ...
        # -------------------------- #
            ...
            self.WebCam_err, img = self.SnapshotAndConv(values)
            if self.WebCam_err != 0:
                WebCamError(self.WebCam_err)
                return
            
            self.IMAGE_bin = img.copy()  # 変換した画像を保存する。
            cv2.imshow('Snapshot', img) # 画像として出力

            return

        elif event == '-Contours-':
            self.WebCam_err, img = Snapshot(self.capture) # 静止画を撮影する。
            if self.WebCam_err != 0:
                WebCamError(self.WebCam_err)
                return

            self.IMAGE_org = img.copy()  # 変換した画像を保存する。

            # 輪郭の階層情報の保持方法を選択
            kernalShape = KernelShapeOption(values['-KernelShape-'])
            if kernalShape is None:
                sg.popup('選択したカーネルの形は存在しません。', title='エラー')
            ...
            if G is None:
                sg.popup('重心が求められませんでした。', title='エラー')
                return
            ...
            self.Window['-CenterOfGravity_x-'].update(str(G[0]))
            self.Window['-CenterOfGravity_y-'].update(str(G[1]))
            return
...
def TwoThreshold(img, gaussian=False, LowerThreshold=0, UpperThreshold=128, PickupColor=4, Type=cv2.THRESH_BINARY):
    ...
    # for Red
    IMAGE_R_bw = GlobalThreshold(r, gaussian, LowerThreshold, Type)
    IMAGE_R__ = GlobalThreshold(r, gaussian, UpperThreshold, Type)
    IMAGE_R__ = cv2.bitwise_not(IMAGE_R__)
    # for Green
    IMAGE_G_bw = GlobalThreshold(g, gaussian, LowerThreshold, Type)
    IMAGE_G__ = GlobalThreshold(g, gaussian, UpperThreshold, Type)
    IMAGE_G__ = cv2.bitwise_not(IMAGE_G__)
    # for Blue
    IMAGE_B_bw = GlobalThreshold(b, gaussian, LowerThreshold, Type)
    IMAGE_B__ = GlobalThreshold(b, gaussian, UpperThreshold, Type)
