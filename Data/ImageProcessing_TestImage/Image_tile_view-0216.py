...
#----------------------
f_path = os.getcwd()
input_path = f_path + '\\*' + search_image
output_path = f_path + '\\' + str(output_name)
f_list = glob.glob(input_path) # カレントディレクトリ内の画像の全てのパスを取得: list

ff_list = []
sub_list = []
for d, s, f in os.walk(f_path):
    if d in search_image: # カレントディレクトリにsearch_imgと同じ拡張子のファイルが存在する場合
        for i in d:
            while True:
                if type(i) != list: break
                else: sub_list.append(i)
