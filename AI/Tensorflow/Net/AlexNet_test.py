...
def LeNet(x, keep_prob):
    x = tf.layers.conv2d(inputs=x, filters=6, kernel_size=[
                         5, 5], strides=1, padding='valid', activation=None, name='conv1')
    x = tf.nn.sigmoid(tf.layers.max_pooling2d(x, pool_size=[2, 2], strides=2))
    x = tf.layers.conv2d(inputs=x, filters=16, kernel_size=[
                         5, 5], strides=1, padding='valid', activation=None, name='conv2')
    x = tf.nn.sigmoid(tf.layers.max_pooling2d(x, pool_size=[2, 2], strides=2))
...
def train():
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(500):
            ...
            _, acc, los = sess.run([train, accuracy, loss], feed_dict={
                                   X: x, Y: t, keep_prob: 0.5})
