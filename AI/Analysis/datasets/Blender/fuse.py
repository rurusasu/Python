...
def _prepare_dataset_single(
    ...
    for obj_id, obj_name in enumerate(obj_names):
        image_dbs[obj_id] = _collect_linemod_set_info(
    ...
    for obj_id, _ in enumerate(obj_names):
        rgb, mask, begin, pose = __randomly_sample_foreground(
            image_dbs[obj_id], pvnet_linemod_dir
        )
        mask *= obj_id + 1
        rgbs.append(rgb)
        masks.append(mask)
...
def __collect_train_val_test_info(pvnet_linemod_dir: str, obj_name: str) -> list:
    """
...
def _fuse_regions(
    ...
    for idx in fuse_order:
        ...
        fuse_img[bh : bh + rh, bw : bw + rw] *= out_silhouette.astype(fuse_img.dtype)[
            :, :, None
        ]
