...
def add_shader_on_world(wld_dat=bpy.data.worlds["World"]):
    """
...
def parse_argument():
    ...
    parser.add_argument(
        "--input",
        type=str,
        default="./doc/car/models/model_normalized.obj",
...
def render(camera, outfile, pose):
    ...
    # phi = float(elevation_deg) / 180 * math.pi
    # theta = float(azimuth_deg) / 180 * math.pi
    # cx = dist * math.cos(theta) * math.cos(phi)
    # cy = dist * math.sin(theta) * math.cos(phi)
    # cz = dist * math.sin(phi)
    # --------------
...
def set_material_node_parameters(material, input):
    nodes = material.node_tree.nodes
    if os.path.basename(input).endswith(".ply"):
...
def setup():
    ...
    b_empty = parent_obj_to_camera(cam_obj)
...
def batch_render_with_linemod(input_ply, output_dir, bg_imgs_npy, poses_path, camera):
    os.system("mkdir -p {}".format(output_dir))
    bpy.ops.import_mesh.ply(filepath=input_ply)
    object = bpy.data.objects[os.path.basename(input_ply).replace(".ply", "")]
    ...
    bg_imgs = np.load(bg_imgs_npy).astype(np.str)
    bg_imgs = np.random.choice(bg_imgs, size=cfg.NUM_SYN)
    poses = np.load(poses_path)
    begin_num_imgs = len(glob.glob(os.path.join(output_dir, "*.jpg")))
    for i in range(begin_num_imgs, cfg.NUM_SYN):
        ...
        render(camera, "{}/{}".format(output_dir, i), pose)
        ...
        with open("{}/{}_RT.pkl".format(output_dir, i), "wb") as f:
...
if __name__ == "__main__":
    ...
    input_ply = "/media/miki/DATA/My_programing/python/AI/Analysis/data/PVNet_linemod/ape/ape.ply"  # obj_path
    output_dir = (
        "/media/miki/DATA/My_programing/python/AI/Analysis/temp/"  # output_dir_path
    )
    bg_imgs_npy_path = (
        "/media/miki/DATA/My_programing/python/AI/Analysis/temp/bg_img_pths.npy"
    )
    poses_path = "/media/miki/DATA/My_programing/python/AI/Analysis/temp/blender_poses/ape_poses.npy"

    obj_name = os.path.basename(input_ply).replace(".ply", "")
    # 全オブジェクトの名前のリスト
    name_list = [obj.name for obj in bpy.data.objects if re.match(obj_name, obj.name)]
    ...
    mat = add_shader_on_ply_object(object)

    bg_imgs = np.load(bg_imgs_npy_path).astype(str)
    # bg_imgs = np.random.choice(bg_imgs, size=cfg.NUM_SYN)
    bg_imgs = np.random.choice(bg_imgs, size=10)
    poses = np.load(poses_path)
    begin_num_imgs = len(glob(os.path.join(output_dir, "*.jpg")))

    # for i in range(begin_num_imgs, cfg.NUM_SYN):
    for i in range(begin_num_imgs, 10):
        img_name = os.path.basename(bg_imgs[i])
        bpy.data.images.load(bg_imgs[i])
        # overlay an background image and place the object
        bpy.data.worlds["World"].node_tree.nodes[
        ...
        set_material_node_parameters(mat, input_ply)
        # render(camera, "{}/{}".format(output_dir, i), pose)

        # --------------
        # Render
        # --------------
        out_file = "{}/{}".format(output_dir, i)
