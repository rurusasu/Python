...
def resize_keep_aspect_ratio(img: np.ndarray, imsize: int, intp_type=cv2.INTER_LINEAR):
    """アスペクト比を保ったままリサイズする関数

    Args:
        img (np.ndarray): 入力画像
        imsize (int): リサイズ後のサイズ
        intp_type ([cv2], optional): リサイズに使用する関数. Defaults to cv.INTER_LINEAR.
    """
...
def rotate(img: np.ndarray, mask: np.ndarray, hcoords, rot_ang_min, rot_ang_max):
    h, w = img.shape[0], img.shape[1]
...
def rotate_instance(
    img: np.ndarray, mask: np.ndarray, hcoords, rot_ang_min, rot_ang_max
):
    """[summary]
    ...
    Args:
        ...
        hcoords ([type]): [description]
        rot_ang_min ([type]): [description]
        rot_ang_max ([type]): [description]
...
def crop_or_padding(img, mask, hcoords, hratio, wratio):
    ...
    out_mask[ohbeg : ohbeg + hlen, owbeg : owbeg + wlen] = mask[
        ihbeg : ihbeg + hlen, iwbeg : iwbeg + wlen
    ]
...
def crop_or_padding_to_fixed_size_instance(
    img, mask, hcoords, th, tw, overlap_ratio=0.5
):
    h, w, _ = img.shape
    hs, ws = np.nonzero(mask)
    ...
    hrmax = int(
        min(hmin + overlap_ratio * fh, h - th)
    )  # h must > target_height else hrmax<0
    hrmin = int(max(hmin + overlap_ratio * fh - th, 0))
    wrmax = int(
        min(wmin + overlap_ratio * fw, w - tw)
    )  # w must > target_width else wrmax<0
    wrmin = int(max(wmin + overlap_ratio * fw - tw, 0))

    hbeg = 0 if hpad else np.random.randint(hrmin, hrmax)
    hend = hbeg + th
    wbeg = (
        0 if wpad else np.random.randint(wrmin, wrmax)
...
def mask_out_instance(img, mask, min_side=0.1, max_side=0.3):
    ...
    img[
        y_loc - y_side : y_loc + y_side, x_loc - x_side : x_loc + x_side
    ] = np.random.randint(
        0,
        255,
        img[y_loc - y_side : y_loc + y_side, x_loc - x_side : x_loc + x_side].shape,
    )
