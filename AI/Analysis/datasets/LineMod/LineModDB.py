...
class LineModDB(object):
    ...
    def __init__(self, linemod_dir: str, obj_name: str = "all"):
        """
            ...
            length = len(
                list(
                    filter(
                        ...
                        os.listdir(os.path.join(self.base_dir, obj_name, "data")),
                    )
                )
            )
            self.lengths[obj_name] = length
            self.total_length += length
        self.model_path = os.path.join(linemod_dir, "{}/mesh.ply".format(obj_name))
        self.old_model_path = os.path.join(
            linemod_dir, "{}/OLDmesh.ply".format(obj_name)
        )
        self.transform_dat_path = os.path.join(
            self.base_dir, "{}/transform.dat".format(obj_name)
        )
    ...
    def load_ply_model(self) -> np.ndarray:
        ...
        if os.path.exists(self.model_path):
            return read_ply_model(self.model_path) / 1000.0
        else:
            transform = read_transform_dat(self.transform_dat_path)
            old_model = read_ply_model(self.old_model_path) / 1000.0
    ...
    def __getitem__(self, idx):
        ...
        for obj_name in self.obj_names:
            if local_idx < self.lengths[obj_name]:
                ...
                # image
                img_pth = os.path.join(dat_dir, "color{}.jpg".format(local_idx))
                img = transforms.ToTensor()(Image.open(img_pth).convert("RGB"))
                pose = read_pose(
                    os.path.join(dat_dir, "rot{}.rot".format(local_idx)),
...
if __name__ == "__main__":
    ...
    # インスタンス生成 #
    # ----------------------- #
    db = LineModDB(linemod_dir=base_dir, obj_name=obj_name)
