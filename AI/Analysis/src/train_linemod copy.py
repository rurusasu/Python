...
from src.model.model_reposnum_itory import *
...
wnum_ith open(args.cfg_file, "r") as f:
...
def train(net, optimizer, dataloader, epoch):
    ...
    def is_to_eval(epoch: int, num_it: int):
        """評価の周波数を決めるための関数

        Args:
            epoch (int): 最大 Epoch 数
            num_it (int): イテレーション数
        """
        if num_it < 300 * 100:
        ...
        to_eval = (num_it % eval_frequency) == 0
    ...
    num_it = start_it
    _, eval_frequency = is_to_eval(epoch, num_it)
    ...
    for epoch in tbar:
        ...
        for idx, batch in enumerate(dataloader):
            ...
            # 最適化対象のすべてのテンソルの勾配を0で初期化する
            # REF: https://teratail.com/questions/261005
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            # プログレスバーを更新する
            pbar.update()
            pbar.set_postfix(dict(total_it=num_it))
            tbar.refresh()
