...
class LineModImageDB(object):
    ...
    def __init__(
        ...
        if has_render_set:
            self.render_pkl = os.path.join(
                self.linemod_dir, "posedb", "{}_render.pkl".format(object_name)
            )
            # prepare dataset
        ...
        self.real_pkl = os.path.join(
            self.linemod_dir, "posedb", "{}_real.pkl".format(object_name)
        )
        if os.path.exists(self.real_pkl):
            ...
            if has_fuse_set:
                self.fuse_pkl = os.path.join(
                    cfg.LINEMOD, "posedb", "{}_fuse.pkl".format(object_name)
                )
                # prepare dataset
    ...
    def collect_render_set_info(self, pkl_file, render_dir, format="jpg"):
        ...
        for k in range(self.render_num):
            ...
            data["RT"] = read_pickle(
                os.path.join(self.linemod_dir, render_dir, "{}_RT.pkl".format(k))
            )["RT"]
