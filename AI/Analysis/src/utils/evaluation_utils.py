...
class Evaluator(object):
    ...
    def cm_degree_5_metric(self, pose_pred, pose_targets):
        ...
        self.cm_degree_5_recorder.append(
            translation_distance < 5 and angular_distance < 5
        )
    ...
    def evaluate_uncertainty_v2(
        ...
        pose_pred = uncertainty_pnp_v2(
            mean_pts2d, covar, points_3d, self.projector.intrinsic_matrix[intri_type]
        )
        model = self.linemod_db.get_ply_model(class_type)
        diameter = self.linemod_db.get_diameter(class_type)

        if class_type in ["eggbox", "glue"]:
            self.projection_2d_sym(
                pose_pred,
                pose_targets,
                model,
    ...
    def average_precision(self, verbose=True):
        ...
        if verbose:
            print(
                "2d projections metric: {}".format(np.mean(self.projection_2d_recorder))
            )
