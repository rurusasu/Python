...
def visualize_bounding_box(
    rgb,
    corners_pred,
    corners_targets=None,
    centers_pred=None,
    centers_targets=None,
    save=False,
    save_fn=None,
):
    """
    ...
    for idx in range(batch_size):
        ...
        ax.add_patch(
            patches.Polygon(
                xy=corners_pred[idx, 0][[0, 1, 3, 2, 0, 4, 6, 2]],
                fill=False,
                linewidth=1,
                edgecolor="b",
            )
        ...
        ax.add_patch(
            patches.Polygon(
                xy=corners_pred[idx, 0][[5, 4, 6, 7, 5, 1, 3, 7]],
                fill=False,
                linewidth=1,
                edgecolor="b",
            )
        ...
        if corners_targets is not None:
            ax.add_patch(
                patches.Polygon(
                    xy=corners_targets[idx, 0][[0, 1, 3, 2, 0, 4, 6, 2]],
                    fill=False,
                    linewidth=1,
                    edgecolor="g",
                )
            ...
            ax.add_patch(
                patches.Polygon(
                    xy=corners_targets[idx, 0][[5, 4, 6, 7, 5, 1, 3, 7]],
                    fill=False,
                    linewidth=1,
                    edgecolor="g",
                )
...
def visualize_mask(mask_pred, mask_gt, save=False, save_fn=None):
    ...
    for bi in range(b):
        ...
        img[np.logical_and(np.logical_not(mask_gt[bi]), mask_pred[bi])] = np.asarray(
            [[[255, 255, 0]]]
        )
        img[np.logical_and(np.logical_not(mask_pred[bi]), mask_gt[bi])] = np.asarray(
            [[[255, 0, 0]]]
        )
        plt.imshow(img)
        if not save:
...
def visualize_points_3d(pts1, pts2, K, h=480, w=640):
    ...
    def get_img(pts_img1, pts_img2):
        img = np.zeros([h, w, 3], np.uint8)
        img[np.logical_and(pts_img1 > 0, pts_img2 > 0)] = np.asarray(
            [0, 255, 0], np.uint8
        )
        img[np.logical_and(pts_img1 > 0, pts_img2 == 0)] = np.asarray(
            [255, 255, 0], np.uint8
...
def visualize_hypothesis(
    rgb, hyp_pts, hyp_counts, pts_target, save=False, save_fn=None
):
    """
    :param rgb:         b,h,w
    ...
    for bi in range(b):
        for vi in range(vn):
            ...
            mask = np.logical_and(
                np.logical_and(cur_hyp_pts[:, 0] > 0, cur_hyp_pts[:, 0] < w * 1.0),
            ...
            colors = (
                cur_hyp_counts / cur_hyp_counts.max()
            )  # [:,None]#*np.array([[255,0,0]])
            plt.figure(figsize=(10, 8))
            plt.imshow(rgb[bi])
            plt.scatter(
                cur_hyp_pts[:, 0], cur_hyp_pts[:, 1], c=colors, s=0.1, cmap="viridis"
            )
...
def visualize_vertex(vertex, vertex_weights, save=False, save_fn=None):
    if type(vertex) == Tensor:
        vertex = vertex.to('cpu').detach().numpy().copy()
    if type(vertex_weights) == Tensor:
        vertex_weights = vertex_weights.to('cpu').detach().numpy().copy()

    vertex = vertex * vertex_weights
    for bi in range(9):
        _, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 8))
        #ax1.imshow(vertex[0, 2 * bi].cpu())
        #ax2.imshow(vertex[0, 2 * bi + 1].cpu())
        ax1.imshow(vertex[0, 2 * bi])
        ax2.imshow(vertex[0, 2 * bi + 1])
        if save:
            plt.savefig(save_fn.format(bi))
        else:
            plt.show()
        ...
        for vi in range(vn):
            ...
            for k in range(num):
                ...
                elp = patches.Ellipse(
                    ...
                    size[0],
                    size[1],
                    np.arctan2(v[1, 0], v[0, 0]) / np.pi * 180,
                    fill=False,
                    color=yellow / num * (num - k) + red / num * k,
                )
                ax.add_patch(elp)
...
def visualize_keypoints(rgb, pts_target, pts_pred=None, save=False, save_fn=None):
    ...
    for bi in range(batch_size):
        ...
        ax.imshow(rgb[bi])
        ax.scatter(
            pts_target[bi, :, 0], pts_target[bi, :, 1], c=np.arange(pts_target.shape[1])
        )
        if pts_pred is not None:
            ax.scatter(
                pts_pred[bi, :, 0], pts_pred[bi, :, 1], c=np.arange(pts_pred.shape[1])
            )
        if not save:
            plt.show()
        else:
