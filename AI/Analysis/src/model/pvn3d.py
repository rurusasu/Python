...
class Pointnet2MSG(nn.Module):
    ...
    def __init__(self, input_channels: int = 6, use_xyz: bool = True):
...
class PVN3D(nn.Module):
    ...
    def __init__(
        ...
        self.pointnet2 = Pointnet2MSG(
            input_channels=pcld_input_channels, use_xyz=pcld_use_xyz
        )
    ...
    def forward(self, pointcloud, rgb, choose):
        ...
        pred_kp_of = self.KpOF_layer(rgbd_feature).view(bs, self.num_kps, 3, N)
        # return [bs, n_kps, n_pts, c]
        pred_kp_of = pred_kp_of.permute(0, 1, 3, 2).contiguous()
        pred_ctr_of = self.CtrOf_layer(rgbd_feature).view(bs, 1, 3, N)
