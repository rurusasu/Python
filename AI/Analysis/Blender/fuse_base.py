...
class ModelAligner(object):
    ...
    def __init__(self, obj_name,pvnet_linemod_dir,linemod_dir):
        ...
        self.orig_old_model_path = os.path.join(linemod_dir,'{}/OLDmesh.ply'.format(obj_name))
        self.transform_dat_path = os.path.join(linemod_dir,'{}/transform.dat'.format(obj_name))
...
def collect_train_val_test_info(pvnet_linemod_dir,obj_name):
...
def collect_linemod_set_info(pvnet_linemod_dir,linemod_obj_name,linemod_dir,cache_dir='./'):
    database=[]
    if os.path.exists(os.path.join(cache_dir,'{}_info.pkl').format(linemod_obj_name)):
        return read_pickle(os.path.join(cache_dir,'{}_info.pkl').format(linemod_obj_name))

    _,train_fns=collect_train_val_test_info(pvnet_linemod_dir,linemod_obj_name)
    print('begin generate database {}'.format(linemod_obj_name))
    rgb_dir=os.path.join(pvnet_linemod_dir,linemod_obj_name,'JPEGImages')
    msk_dir=os.path.join(pvnet_linemod_dir,linemod_obj_name,'mask')
    rt_dir = os.path.join(linemod_dir, linemod_obj_name, 'data')
    ...
    for k in range(img_num):
        ...
        pose_transformer = PoseTransformer(linemod_obj_name, pvnet_linemod_dir, linemod_dir)
        data['RT'] = pose_transformer.orig_pose_to_blender_pose(pose).astype(np.float32)
        database.append(data)

    print('success generate database {} len {}'.format(linemod_obj_name,len(database)))
    save_pickle(database,os.path.join(cache_dir,'{}_info.pkl').format(linemod_obj_name))
    return database
...
def prepare_dataset_parallel(output_dir, pvnet_linemod_dir, linemod_dir, fuse_num, background_dir, cache_dir, worker_num=8):
    exector=ProcessPoolExecutor(max_workers=worker_num)
    futures=[]

    for obj_name in linemod_obj_names:
        collect_linemod_set_info(pvnet_linemod_dir,obj_name,linemod_dir,cache_dir)
    ...
    for idx in np.arange(fuse_num):
        ...
        futures.append(exector.submit(
            prepare_dataset_single,output_dir,idx, linemod_obj_names, pvnet_linemod_dir, linemod_dir, background_dir,cache_dir, seed))

    for f in futures:
        f.result()


def prepare_dataset_single(output_dir,idx,linemod_obj_names,pvnet_linemod_dir,linemod_dir,background_dir,cache_dir,seed):
    time_begin=time.time()
    np.random.seed(seed)
    rgbs,masks,begins,poses=[],[],[],[]
    image_dbs={}
    for cls_id,obj_name in enumerate(linemod_obj_names):
        image_dbs[cls_id]=collect_linemod_set_info(pvnet_linemod_dir,obj_name,linemod_dir,cache_dir)

    for cls_id,obj_name in enumerate(linemod_obj_names):
        rgb, mask, begin, pose=randomly_sample_foreground(image_dbs[cls_id], pvnet_linemod_dir)
        mask*=cls_id+1
        rgbs.append(rgb)
        masks.append(mask)
        begins.append(begin)
...
def randomly_sample_foreground(image_db,pvnet_linemod_dir):
...
linemod_obj_names=['ape','cam','cat','duck','glue','iron','phone', 'benchvise','can','driller','eggbox','holepuncher','lamp']


def run():
    output_dir='./data/LINEMOD/fuse/'
    pvnet_linemod_dir=cfg.LINEMOD
    linemod_dir=cfg.LINEMOD_ORIG
...
if __name__=="__main__":
    output_dir='tmp/'
    pvnet_linemod_dir='/home/liuyuan/data/LINEMOD'
    linemod_dir='/home/liuyuan/data/LINEMOD_ORIG'
