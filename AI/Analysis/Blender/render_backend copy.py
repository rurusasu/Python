...
def add_shader_on_world():
    ...
    bpy.data.worlds["World"].node_tree.links.new(
        env_node.outputs["Color"], back_node.inputs["Color"]
    )
...
def add_shader_on_obj_object(obj):
    ...
    material.node_tree.links.new(
        diffuse_node.outputs["BSDF"], mat_out.inputs["Surface"]
    )
    material.node_tree.links.new(
        image_node.outputs["Color"], diffuse_node.inputs["Color"]
    )
