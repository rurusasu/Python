...
def FlowChartPrint_click():
    ...
    aspect = (abs(ymax-ymin))/(abs(xmax-xmin))*abs(ax1.get_xlim()[1] - ax1.get_xlim()[0]) \
    ...
    for key in model.sequential.keys():
        a = a+[key]
...
def Flow_click(flow_data):
    y = model.flow(flow_data)
...
while True:
    ...
    #---------------------------------
        ...
        InLayer = LayerAdd_click(layerName=values['-LayerName-'],
                                 Node=values['-Node-'],
