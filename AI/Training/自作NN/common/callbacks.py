...
class LearningVisualizationCallback:
    ...
    def __init__(self, higher_better_metrics, fig=None, ax=None):
        ...
        self._metric_history_lines = {} # 関数ごとに渡された値を格納するための辞書
        self._higher_better_metrics = set(higher_better_metrics) # ベストスコアを見たい関数を設定する
        self._metric_type_higher_better = defaultdict(bool)      #ベストスコアを見たい関数が入力ラベル内にあるか判定　あればTrue
        self._best_score_vlines = {} # ベストスコアの縦線の設定を保存するための辞書
    ...
    def one_epoch_end(self, logs=None):
        """各エポック毎に呼ばれるコールバック"""
        ...
        for metric, score in logs.items():
            ...
            # 初回だけの設定
            if len(self._metric_histories[metric]) == 1:
                # メトリックの種別を保存する
