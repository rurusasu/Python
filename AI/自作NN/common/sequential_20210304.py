...
class Sequential:
    ...
    def printParams(self):
        for num in range(len(self.sequential)):
            print(
                "レイヤ："
                + str(self.sequential[num + 1])
                + "ユニット数："
                + str(self.sequential[num + 1].units)
            )
    ...
    def fit(self, x, t, batch_size, epochs, validation=None, callbacks=None):
        ...
        for epoch in range(epochs):
            ...
            for key, List in self.logs.items():
                ...
                if "train" in key:
                    ...
                    acc_train = self.accuracy(x_batch, t_batch, self.func[metric])
            ...
            print(
                "学習%d回目  --loss:%f, --val=%f, --train_acc=%f, --val_acc=%f"
                % (
                    epoch + 1,
                    self.history["loss_ave"][epoch],
                    self.history["val_loss"][epoch],
                    np.min(prt_train_acc),
                    np.min(prt_val_acc),
                )
            )

        nb_epoch = epochs
        plt.figure()  # 新しいFigureオブジェクトを作成
        plt.plot(range(nb_epoch), self.history["loss_ave"], marker=".", label="loss")
        plt.plot(
            range(nb_epoch), self.history["val_loss"], marker=".", label="val_loss"
        )
        ...
        print(
            "loss=%f, val=%f"
            % (
                self.history["loss_ave"][epochs - 1],
...
if __name__ == "__main__":
    ...
    # 訓練データの読み込み
    x_train = np.loadtxt(
        cfg.DATA_DIR + os.sep + "training.csv",  # 読み込むファイル名(例"save_data.csv")
    ...
    # テストデータの読み込み
    t_train = np.loadtxt(
        cfg.DATA_DIR + os.sep + "test.csv",  # 読み込むファイル名(例"save_data.csv")
        dtype=float,  # データのtype
        delimiter=",",  # 区切り文字の指定
        ndmin=2,  # 配列の最低次元
    )
    ...
    x_val = np.loadtxt(
        cfg.DATA_DIR + os.sep + "val_l.csv",  # 読み込むファイル名(例"save_data.csv")
        dtype=float,  # データのtype
        delimiter=",",  # 区切り文字の指定
        ndmin=2,  # 配列の最低次元
    )
    ...
    t_val = np.loadtxt(
        cfg.DATA_DIR + os.sep + "val_t.csv",  # 読み込むファイル名(例"save_data.csv")
        dtype=float,  # データのtype
        delimiter=",",  # 区切り文字の指定
        ndmin=2,  # 配列の最低次元
    )
