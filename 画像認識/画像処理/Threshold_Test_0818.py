...
# 画像を要素毎に分解
# ----------------------------------
def Disassembly(src: np.ndarray) -> list:
    """
    画像を画素ごとに要素に分解する関数
...
def Twothresh_binalize(
    src, LowerThreshold=0, UpperThreshold=128, PickupColor=4, Type=cv2.THRESH_BINARY
):
    r, g, b = cv2.split(src)
...
while True:  # The PSG "Event Loop"
    ...
    if event == None:
        ...
        window["-Image_org-"].update(data=cv2.imencode(".png", src)[1].tobytes())
        # Update image in window
        window["-Im_Glay-"].update(data=cv2.imencode(".png", src_glay)[1].tobytes())
        ...
        for i, src_bin in enumerate(dst_elem):  # 画像を3つの要素に分解
            ...
            if values["-Binary_Type-"] == "Global":
                ...
                ret, dst = Twothresh_binalize(
                    src_bin,
                    LowerThreshold=values["-LowerThresh-"],
                    UpperThreshold=values["-UpperThresh-"],
                    PickupColor=pickup,
                )
            window[window_num].update(
                data=cv2.imencode(".png", src_bin)[1].tobytes()
            )  # Update image in window
            window[window_num_bin].update(
                data=cv2.imencode(".png", dst)[1].tobytes()
            )  # Update image in window
