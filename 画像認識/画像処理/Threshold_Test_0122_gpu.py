...
def LUT_curve(f, a, src):
    ...
    out_rgb_img = cv2.cuda.LUT(src, LUT)
    return out_rgb_img
...
while True:                     # The PSG "Event Loop"
    ...
    ret, frame = cap.read()
    img_gpu_src.upload(frame) # ホスト->デバイスへのメモリ転送
    
    img_gpu_dst_1 = img_gpu_src.copy()
    # bgr → glay
    img_gpu_dst_2 = cv2.cuda.cvtColor(img_gpu_dst_1, cv2.COLOR_BGR2GRAY)
    img_gpu_dst_3 = img_gpu_dst_2.copy()
    # gamma変換
    img_gpu_dst_3 = gamma_Image(img_gpu_dst_3, 26)
    #------------------------------------------------
    # 画面上に撮影した画像のヒストグラムを表示する
    #------------------------------------------------
    src_1 = img_gpu_dst_1.download()
    src_2 = img_gpu_dst_2.download()
    src_3 = img_gpu_dst_3.download()
